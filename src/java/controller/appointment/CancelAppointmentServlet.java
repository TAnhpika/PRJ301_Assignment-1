...
public class CancelAppointmentServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<model.Appointment> appointments = new dal.AppointmentDAO().getAllAppointmentsWithDetails();
    ...
    private void handleGetTimeSlotsForStaff(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            java.util.List<model.TimeSlot> availableSlots = dal.TimeSlotDAO.getAvailableSlotsByDoctorAndDate(doctorId, workDate);
            System.out.println("[DEBUG] availableSlots: " + (availableSlots != null ? availableSlots.size() : "null"));
            java.util.List<Integer> bookedSlotIds = dal.AppointmentDAO.getBookedSlots(doctorId, localDate);
...
//                    boolean success = false;
                    if (slotIdStr != null && workDate != null) {
                        ...
                        success = dal.AppointmentDAO.updateAppointmentForReschedule(appointmentId, workDate, slotId);
                    }
                    if (success) {
                    ...
                    success = dal.AppointmentDAO.updateAppointmentStatusStatic(appointmentId, dal.AppointmentDAO.STATUS_CANCELLED);
                    if (success) {
                        model.Appointment ap = dal.AppointmentDAO.getAppointmentWithPatientInfo(appointmentId);
                        String[] emails = dal.AppointmentDAO.getEmailsFromAppointment(appointmentId);
